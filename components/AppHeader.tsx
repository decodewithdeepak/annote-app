...
export default function AppHeader() {
  const { audioSource, resetAll, segments, exported, handleExport } = useAudio();
  ...
  return (
    <header className="flex shrink-0 items-center justify-between border-b border-white/10 bg-white/5 px-4 py-3 sm:px-7">
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs font-bold tracking-[0.3em] text-white">WAVEMARK</span>
        <span className="hidden text-xs tracking-wide text-white/30 sm:inline">Audio Annotation Studio</span>
      </div>

      <div className="flex items-center gap-3">
        {hasAudio && (
          <>
            <span className="hidden max-w-50 truncate text-xs text-white/35 md:inline">{audioSource.file?.name}</span>
            <button
              type="button"
